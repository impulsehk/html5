...
//Column titles
$header = array('Booking ID', 'Name','Phone','Checkin Date','Checkout Date','Amount','Payment Method');
$hotelid=$bsiCore->ClearInput($_GET['hotelid']);
$booking_list_type=$bsiCore->ClearInput($_GET['booking_list_type']);
// Sql Query................

	$viewarchivbookresult=mysql_query("select  bb.booking_id,CONCAT(bc.first_name,' ',bc.surname) as Name,bc.phone,DATE_FORMAT(bb.checkin_date, '".$bsiCore->userDateFormat."') AS checkin_date,DATE_FORMAT(bb.checkout_date, '".$bsiCore->userDateFormat."') AS checkout_date,bb.total_cost,bpg.gateway_name
			from bsi_bookings as bb,bsi_clients as bc, bsi_payment_gateway as bpg 
			where bb.client_id=bc.client_id and bpg.gateway_code=bb.payment_type and
			bb.hotel_id='".$hotelid."' and 
			(checkout_date > curdate() and bb.is_deleted='".$booking_list_type."')") or die("Error at line : 11".mysql_error());
