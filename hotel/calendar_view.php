...
<button class="button_colour round_all" type="button" name="refresh" id="refresh" style="float:right;"><img height="24" width="24" alt="Bended Arrow Right" src="../admin/images/icons/small/white/bended_arrow_right.png"><span>Refresh</span></button>
  <h2>Calendar View Availability</h2>
</div>
<form action="<?=$_SERVER['PHP_SELF']?>" method="post">
<div class="box grid_16 round_all">
  <h2 class="box_head grad_colour round_top"><?php if(isset($_SESSION['hhid'])){?>&nbsp; <?=$bsiAdminMain->getYearcombo($cYear)?> &nbsp; <?php if(isset($_POST['submit'])){ echo $bsiAdminMain->getRoomtypeCal($_SESSION['hhid'], $_POST['roomtype']); }else{ echo $bsiAdminMain->getRoomtypeCal($_SESSION['hhid']); } }?>&nbsp; <?php if(isset($_SESSION['hhid'])){?><?=$buttonhtml?></h2>
      ...
      <tr>
        ...
        <?php
			...
			foreach($monthNames as $key => $month){ 
				...
				$row = mysql_fetch_assoc(mysql_query("SELECT count(*) AS no_of_room FROM `bsi_room` where hotel_id=".$_SESSION['hhid']." ".$sql));
				...
				for ($i=0; $i<($maxday+$startday); $i++) {
				   if(isset($_POST['submit'])){
					   $result = mysql_query("SELECT count(br.roomtype_id) as counter FROM bsi_bookings as bb, bsi_reservation as br, bsi_room as bro, bsi_roomtype as brt WHERE bb.booking_id = br.booking_id and bro.room_id = br.room_id and bro.roomtype_id = brt.roomtype_id and '".$date_form.($i - $startday + 1)."' between bb.checkin_date and DATE_SUB(bb.checkout_date, INTERVAL 1 DAY) and bb.hotel_id=".$_SESSION['hhid']." and br.roomtype_id=".$_POST['roomtype']);
				   }else{
					   $result = mysql_query("SELECT count(br.roomtype_id) as counter FROM bsi_bookings as bb, bsi_reservation as br, bsi_room as bro, bsi_roomtype as brt, bsi_capacity as bc WHERE bb.booking_id = br.booking_id and bro.room_id = br.room_id and bro.roomtype_id = brt.roomtype_id and bro.capacity_id = bc.capacity_id and '".$date_form.($i - $startday + 1)."' between bb.checkin_date and DATE_SUB(bb.checkout_date, INTERVAL 1 DAY) and bb.hotel_id=".$_SESSION['hhid']);
				   }
