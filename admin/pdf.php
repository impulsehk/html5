<?php
	...
	if(isset($_SESSION['shortby'])){
		$sqlQuery="SELECT booking_id, CONCAT(bsi_clients.first_name,' ',bsi_clients.surname) as Name, bsi_clients.phone, DATE_FORMAT(checkin_date, '".$bsiCore->userDateFormat."') AS start_date, DATE_FORMAT(checkout_date, '".$bsiCore->userDateFormat."') AS end_date, total_cost, DATE_FORMAT(booking_time, '".$bsiCore->userDateFormat."') AS booking_time, gateway_name FROM bsi_bookings, bsi_clients, bsi_payment_gateway where bsi_clients.client_id = bsi_bookings.client_id and bsi_payment_gateway.gateway_code=bsi_bookings.payment_type and payment_success=true and CURDATE() <= checkout_date and is_deleted=false and hotel_id=".$_SESSION['hotelidpdf']." and is_block=false and (DATE_FORMAT(".mysql_real_escape_string($_SESSION['shortby']).", '%Y-%m-%d') between '".$bsiCore->getMySqlDate(mysql_real_escape_string($_SESSION['check_in']))."' and '".$bsiCore->getMySqlDate(mysql_real_escape_string($_SESSION['check_out']))."')"; 
	}else{ 
		$sqlQuery="select bb.booking_id, CONCAT(bc.first_name,' ',bc.surname) as Name, bc.phone, DATE_FORMAT(bb.checkin_date, '".$bsiCore->userDateFormat."') AS start_date, DATE_FORMAT(bb.checkout_date, '".$bsiCore->userDateFormat."') AS end_date, bb.total_cost, DATE_FORMAT(booking_time, '".$bsiCore->userDateFormat."') AS booking_time, bpg.gateway_name from bsi_bookings as bb, bsi_clients as bc, bsi_payment_gateway as bpg where bb.client_id=bc.client_id and bpg.gateway_code = bb.payment_type and bb.hotel_id = '".$bsiCore->ClearInput($_GET['hotelid'])."' and (checkout_date > curdate())";
}
...
if(isset($_REQUEST['hotelid'])){
	...
	$pdf->SetHeaderData('', '', ucwords($hotelRow['hotel_name']), 'Active Booking List');
}else{
	$hotelRow = $bsiCore->getHotelDetails($_SESSION['hotelidpdf']);
	$pdf->SetHeaderData('', '', ucwords($hotelRow['hotel_name']), 'Active Booking List - ( '.$_SESSION['check_in'].' TO '.$_SESSION['check_out'].' BY '.ucwords(str_replace("_", " ", $_SESSION['shortby'])).' ) ');
}
